...
export default function ClientMessagesPage() {
  ...
  const ConversationCard = ({ conversation }: { conversation: Conversation }) => {
    const otherParticipant = conversation.otherParticipant || 
      conversation.participants.find(p => p.role === 'ENTERPRISE'); // Pour les clients, l'autre participant est l'entreprise

    const lastMessageTime = conversation.lastMessage 
      ? MessagingService.formatMessageTime(conversation.lastMessage.sentAt)
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => {
          router.push(`/(app)/(client)/(tabs)/messages/${conversation._id}` as any);
        }}
      >
