      ...
      <View className="flex-1 bg-black/50 justify-center items-center px-6">
        ...
        <View style={{ backgroundColor: colors.card, borderRadius: 24, width: '100%', maxWidth: 400, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 10 }}>
          ...
          >
            ...
            <Text style={{ color: colors.textPrimary, fontFamily: 'PlusJakartaSans-Bold', fontSize: 24, marginBottom: 4 }}>
              {isUpgrade ? `${i18n.t('enterprise.subscription.upgrade.switchTo')} ` : `${i18n.t('enterprise.subscription.upgrade.activate')} `}{plan.name}
            </Text>
            <Text style={{ color: colors.textSecondary, fontFamily: 'PlusJakartaSans-Medium', fontSize: 14, lineHeight: 20 }}>
              {isUpgrade 
          ...
          >
            ...
            <View className="p-6 pt-5">
              ...
              <View style={{ backgroundColor: colors.secondary, borderRadius: 16, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
                <Text style={{ color: colors.textSecondary, fontFamily: 'PlusJakartaSans-SemiBold', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 8 }}>
                  {i18n.t('enterprise.subscription.upgrade.price')}
                </Text>
                <View className="flex-row items-baseline">
                  <Text 
                    style={{ fontSize: 30, fontFamily: 'PlusJakartaSans-Bold', color: plan.color }}
                  >
                    {plan.price}
                  </Text>
                  {plan.period && (
                    <Text style={{ color: colors.textSecondary, fontFamily: 'PlusJakartaSans-SemiBold', fontSize: 14, marginLeft: 8 }}>
                      {plan.period}
                    </Text>
                  )}
                </View>
              ...
              {/* Features List */}
              <View className="mb-5">
                <Text style={{ color: colors.textPrimary, fontFamily: 'PlusJakartaSans-Bold', fontSize: 14, marginBottom: 12 }}>
                ...
                {plan.features.map((feature, idx) => (
                  <View key={idx} className="flex-row items-start mb-2.5">
                    ...
                    <Text style={{ flex: 1, color: colors.textSecondary, fontFamily: 'PlusJakartaSans-Medium', fontSize: 14, lineHeight: 20 }}>
                      {feature}
                    </Text>
                  </View>
                ))}
                ...
                <View className="bg-blue-50 rounded-xl p-4 mb-5 border border-blue-100">
                  <View className="flex-row items-start">
                    ...
                    <View className="flex-1">
                      <Text style={{ color: '#1E3A8A', fontFamily: 'PlusJakartaSans-Bold', fontSize: 14, marginBottom: 4 }}>
                        {i18n.t('enterprise.subscription.upgrade.securePayment')}
                      </Text>
                      <Text style={{ color: '#1D4ED8', fontFamily: 'PlusJakartaSans-Medium', fontSize: 12, lineHeight: 18 }}>
              ...
              <View className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                <View className="flex-row items-start">
                  ...
                  <Text style={{ flex: 1, color: '#92400E', fontFamily: 'PlusJakartaSans-Medium', fontSize: 12, lineHeight: 18 }}>
                    {isUpgrade 
          ...
          <View style={{ padding: 24, paddingTop: 12, borderTopWidth: 1, borderTopColor: colors.border, backgroundColor: colors.secondary }}>
            ...
            >
              <Text style={{ color: colors.textPrimary, fontFamily: 'PlusJakartaSans-SemiBold', fontSize: 16 }}>
